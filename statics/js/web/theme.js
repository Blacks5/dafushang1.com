				...
				if (visibleUl.length) {
					var f = $(visibleUl[0]).offset().top,				// get first children offset top
						...
						for (var i = 0; i < visibleUl.length; i++) {
							...
							//if($(visibleUl[i]).hasClass('wideul')){
								//b.push($(visibleUl[i]).offset().top + $(visibleUl[i]).outerHeight(true) - f*0.9);
							//}else{
								//b.push($(visibleUl[i]).offset().top + $(visibleUl[i]).outerHeight(true) - f*0.6);
							//}
						}
			...
			if (carousel.length) {
				...
				function carousel_visible (carousel) {
					var	carouselWidth 	= carousel.parents(".carousel-wrap").eq(0).width(),
						...
						var newCol;
						if (carouselWidth < 1201 && carouselWidth >= 984 && visible > 5) // bigger that content
							newCol = 5;
						else if (carouselWidth < 1201 && carouselWidth >= 984 && visible > 4) // bigger that content
							newCol = 4;
